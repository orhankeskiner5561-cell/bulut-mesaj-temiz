...
# Runtime patch needs regex support.
src=src.replace('from pathlib import Path\n','from pathlib import Path\nimport re\n',1)

# Replace only the brittle analyzeBtn exact-match section in the V9 patch source.
# The source has no numbered comment markers, so use its actual variable markers.
start=src.find('old="$(\'analyzeBtn\').onclick')
end=src.find('\noldline=', start)
if start<0 or end<0:
    raise SystemExit('V9 runner: analyze source block not found')

fixed=r'''pat=r"\$\('analyzeBtn'\)\.onclick=.*?(?=\n\$\('drawBtn'\))"
