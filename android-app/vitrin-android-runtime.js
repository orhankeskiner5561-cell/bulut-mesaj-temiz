(function(){
  ...
  function prepareVideo(video){
    ...
    // Layout ölçülerine dokunmadan, Android'in boş/oynat ikonlu ilk çizimini sakla.
    ...
    const reveal=()=>{
      if(revealed)return;
      revealed=true;
      requestAnimationFrame(()=>video.style.setProperty('opacity','1','important'));
    };
    const seekFirstFrame=()=>{
      try{
        if(video.duration&&isFinite(video.duration)&&video.currentTime===0){
          video.currentTime=Math.min(.06,Math.max(.01,video.duration/1500));
        }
      }catch{}
    };

    if(video.readyState>=2){seekFirstFrame();reveal();}
    else{
      video.addEventListener('loadedmetadata',seekFirstFrame,{once:true});
      video.addEventListener('loadeddata',()=>{seekFirstFrame();reveal();},{once:true});
      video.addEventListener('canplay',reveal,{once:true});
      // Ağır/bozuk medya durumunda ekran sonsuza kadar kapalı kalmasın.
